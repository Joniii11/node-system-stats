...
export interface BatteryInfo {
  ...
  isCharging: boolean;
  percent: number;
  timeRemaining: number; // in minutes, -1 if calculating or unknown
  acConnected: boolean;
  maxCapacity?: number; // in mWh if available
  currentCapacity?: number; // in mWh if available
  voltage?: number; // in mV if available
  cycleCount?: number; // if available
  model?: string;
}
 ...
 * @private
 */
async function getWindowsBatteryInfo(): Promise<BatteryInfo> {
  try {
    // Check if battery is present
    const { stdout: batteryPresent } = await execAsync('wmic path win32_battery get availability /format:list');
    ...
    // Get battery status
    const { stdout: batteryStatus } = await execAsync('wmic path win32_battery get BatteryStatus /format:list');
    ...
    // Get percentage
    const { stdout: batteryPercent } = await execAsync('wmic path win32_battery get EstimatedChargeRemaining /format:list');
    const percent = parseInt(batteryPercent.split('=')[1], 10) || 0;
    
    // Get time remaining
    const { stdout: timeRemaining } = await execAsync('wmic path win32_battery get EstimatedRunTime /format:list');
    const timeRemainingMinutes = parseInt(timeRemaining.split('=')[1], 10) || -1;
    
    // Get additional information if available
    const { stdout: designCapacity } = await execAsync('wmic path win32_battery get DesignCapacity /format:list');
    const maxCapacity = parseInt(designCapacity.split('=')[1], 10) || undefined;
    
    const { stdout: fullChargeCapacity } = await execAsync('wmic path win32_battery get FullChargeCapacity /format:list');
    const currentCapacity = parseInt(fullChargeCapacity.split('=')[1], 10) || undefined;
    
    const { stdout: voltage } = await execAsync('wmic path win32_battery get DesignVoltage /format:list');
    const voltageValue = parseInt(voltage.split('=')[1], 10) || undefined;
    
    const { stdout: modelInfo } = await execAsync('wmic path win32_battery get DeviceID /format:list');
 ...
 * @private
 */
async function getMacOSBatteryInfo(): Promise<BatteryInfo> {
  try {
    ...
    try {
      const { stdout: batteryInfo } = await execAsync('system_profiler SPPowerDataType');
 ...
 * @private
 */
async function getLinuxBatteryInfo(): Promise<BatteryInfo> {
  try {
    // Check if we have a battery
    const { stdout: batteryPath } = await execAsync('find /sys/class/power_supply -name "BAT*" | head -n 1');
    ...
    // Get battery status
    const { stdout: status } = await execAsync(`cat ${basePath}/status`);
    ...
    let percent = 0;
    try {
      const { stdout: energy_now } = await execAsync(`cat ${basePath}/energy_now`);
      const { stdout: energy_full } = await execAsync(`cat ${basePath}/energy_full`);
      ...
      try {
        const { stdout: charge_now } = await execAsync(`cat ${basePath}/charge_now`);
        const { stdout: charge_full } = await execAsync(`cat ${basePath}/charge_full`);
        ...
        try {
          const { stdout: capacity } = await execAsync(`cat ${basePath}/capacity`);
    ...
    try {
      if (!isCharging) {
        const { stdout: power_now } = await execAsync(`cat ${basePath}/power_now`);
        const { stdout: energy_now } = await execAsync(`cat ${basePath}/energy_now`);
    ...
    try {
      const { stdout: energy_full } = await execAsync(`cat ${basePath}/energy_full 2>/dev/null || echo 0`);
    ...
    try {
      const { stdout: energy_now } = await execAsync(`cat ${basePath}/energy_now 2>/dev/null || echo 0`);
    ...
    try {
      const { stdout: voltage_now } = await execAsync(`cat ${basePath}/voltage_now 2>/dev/null || echo 0`);
    ...
    try {
      const { stdout: modelName } = await execAsync(`cat ${basePath}/model_name 2>/dev/null || echo ""`);
      if (modelName.trim() !== '') {
    ...
    try {
      const { stdout: cycle_count } = await execAsync(`cat ${basePath}/cycle_count 2>/dev/null || echo 0`);
