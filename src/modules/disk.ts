...
import { execAsync, formatBytes } from '../utils/util';
 ...
 */
export async function getDiskInfo(diskPath?: string): Promise<DiskDriveInfo[]> {
 ...
 * @private
 */
async function getWindowsDiskInfo(diskPath: string): Promise<DiskDriveInfo[]> {
  ...
  try {
    ...
    const { stdout } = await execAsync(
      `wmic logicaldisk where "DeviceID='${driveLetter.charAt(0)}:'" get FreeSpace,Size /format:csv`
    );
 ...
 * @private
 */
async function getUnixDiskInfo(diskPath: string): Promise<DiskDriveInfo[]> {
  try {
