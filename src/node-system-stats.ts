 ...
 */
export async function getDiskInfo(pathToCheck?: string): Promise<DiskDriveInfo[]> {
  try {
    const isWindows = process.platform === 'win32';
    const result: DiskDriveInfo[] = [];
 ...
 */
export function getLoadAverage(): LoadAverageInfo {
            ...
            if (typeof value === 'object' && value !== null && key.includes('core')) {
              for (const [subKey, subValue] of Object.entries(value as any)) {
                if (subKey.includes('temp') && typeof subValue === 'object' && subValue !== null) {
                  const temp = (subValue as any).input;
